import CreateProduct from "@/components/CreateProduct";
import Form from "@/components/Form";
import Search from "@/components/Search";
import { useClickOutside } from "@/hooks/useClickOutSide";
import { createProductSchema } from "@/schema/schema";
import { usePostProduct } from "@/services/mutation";
import { useProducts } from "@/services/queries";
import { yupResolver } from "@hookform/resolvers/yup";

import { useRef, useState } from "react";
import { useForm } from "react-hook-form";
import { Toaster } from "react-hot-toast";

function MainPage() {
  const [search, setSearch] = useState("");
  const [page, setPage] = useState(1);
  const [openModal, setOpenModal] = useState(false);
  const modalRef = useRef(null);
  const formDet = useForm({ resolver: yupResolver(createProductSchema) });
  useClickOutside(modalRef, () => {
    setOpenModal(false), formDet.reset();
  });
  const { data } = useProducts(search, page);
  const { mutate } = usePostProduct(formDet, setOpenModal);
  const onSubmit = (data) => {
    mutate(data);
  };
  return (
    <div className="pt-4">
      <Search search={search} setSearch={setSearch} setPage={setPage} />
      <CreateProduct onClick={() => setOpenModal(true)} />
      <Toaster />
      {openModal && (
        <Form
          onSubmit={onSubmit}
          formDet={formDet}
          modalRef={modalRef}
          setOpenModal={setOpenModal}
          type="edit"
        />
      )}

      <div className="mt-4 rounded-3xl border border-[#E4E4E4] bg-white">
        <div className="flex w-full items-center justify-between rounded-tl-3xl rounded-tr-3xl bg-[#F2F2F2] px-14 py-6 text-right">
          <span className="w-1/6">نام کالا</span>
          <span className="w-1/6">موجودی</span>
          <span className="w-1/6">قیمت</span>
          <span className="w-3/6">شناسه کالا</span>
        </div>

        <div className="h-[610px] divide-y">
          {data?.data.map((item) => (
            <div
              key={item.id}
              className="flex w-full items-center justify-between px-14 py-4 text-right"
            >
              <span className="w-1/6">{item.name}</span>
              <span className="w-1/6">{item.quantity}</span>
              <span className="w-1/6">{item.price}</span>
              <div className="flex w-3/6 justify-between">
                <span>{item.id}</span>
                <div className="flex gap-2">
                  <span className="cursor-pointer rounded-full p-1 text-green-500 transition-colors duration-150 hover:bg-green-500 hover:text-white">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                      strokeWidth={2}
                      stroke="currentColor"
                      className="size-5"
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10"
                      />
                    </svg>
                  </span>
                  <span className="cursor-pointer rounded-full p-1 text-red-500 transition-colors duration-150 hover:bg-red-500 hover:text-white">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                      strokeWidth={2}
                      stroke="currentColor"
                      className="size-5"
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"
                      />
                    </svg>
                  </span>
                </div>
              </div>
            </div>
          ))}
        </div>
      </div>
      <div>
        <p className="mx-auto">asdasd</p>
      </div>
    </div>
  );
}

export default MainPage;
