  ...
  return (
    <div className="flex h-full items-center justify-center">
      ...
      <form
        ...
        className="flex flex-col items-center justify-center rounded-[40px] border border-[#E4E4E4] bg-white px-6 pb-5 pt-12"
      >
        {/* Logo */}
        <div className="flex flex-col items-center justify-center gap-y-4">
          ...
          <h2 className="text-2xl text-zinc-900">فرم ثبت نام</h2>
        </div>
        {/* Form fileds */}
        <div className="mt-16 flex w-[400px] flex-col items-center justify-between gap-4">
          <div className="h-20 w-full [&>input]:w-full [&>input]:rounded-2xl [&>input]:bg-[#F2F2F2] [&>input]:px-4 [&>input]:py-4 [&>input]:text-gray-700 [&>input]:placeholder:text-[#282828]/50 [&>input]:focus:outline-none">
            <input
              type="text"
        ...
        <button
          disabled={isPending}
          className={`mt-9 flex w-full items-center justify-center gap-3 rounded-2xl bg-[#55A3F0] py-3.5 text-xl text-white ${isPending && "cursor-not-allowed"}`}
        >
          ثبت نام {isPending && <Spinner />}
        </button>
        <Link
          className="ml-auto mt-5 text-start text-[#3A8BED] transition-colors duration-150 hover:text-blue-400"
          to={"/login"}
        >
          حساب کاربری دازید؟
        </Link>
